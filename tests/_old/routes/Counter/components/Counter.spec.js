...
describe('(Component) Counter', () => {
  ...
  it('Should render exactly two buttons.', () => {
    expect(_wrapper.find('RaisedButton')).to.have.length(2);
  });
  ...
  describe('An increment button...', () => {
    ...
    beforeEach(() => {
      _button = _wrapper.find('RaisedButton').filterWhere(a => a.prop('label') === 'Increment');
    });
  ...
  describe('A Double (Async) button...', () => {
    ...
    beforeEach(() => {
      _button = _wrapper.find('RaisedButton').filterWhere(a => a.prop('label') === 'Double (Async)');
    });
