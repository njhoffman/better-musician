...
var colors = {
  depth1: (msg) => ansiColors.fg.grayscale[23] + msg + ansiColors.reset,
  depth2: (msg) => ansiColors.fg.grayscale[18] + msg + ansiColors.reset,
  depth3: (msg) => ansiColors.fg.grayscale[14] + msg + ansiColors.reset,
  depthOther: (msg) => ansiColors.fg.grayscale[10] + msg + ansiColors.reset,
  log: (msg) => ansiColors.fg.getRgb(2,3,4) + msg + ansiColors.reset,
  info: (msg) => ansiColors.fg.getRgb(4,3,2) + msg + ansiColors.reset
};
...
var SpecReporter = function (baseReporterDecorator, formatError, config) {
    ...
    return (function (browser, result) {
      ...
      suite.forEach(function (value, index) {
        if (index >= this.currentSuite.length || this.currentSuite[index] != value) {
          ...
          if (index === 0) {
            value = colors.depth1(value);
          } else if (index === 1) {
            value = colors.depth2(value);
          } else if (index === 1) {
            value = colors.depth3(value);
          } else {
            value = colors.depthOther(value);
          }
  ...
  this.onBrowserLog = doLog ? function (browser, log, type) {
    if (this._browsers && this._browsers.length === 1) {
      this.write(this.LOG_SINGLE_BROWSER, colors.log("[3m" +  type.toLowerCase() + "[0m"), this.USE_COLORS ? log.cyan : log);
    } else {
